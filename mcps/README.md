# Cybersecurity-related MCP server on Databricks Apps

Based on the example from [Databricks Labs MCP project](https://github.com/databrickslabs/mcp).
...
```bash
uvicorn dbx_cyber_mcp.app:app --reload
```
...
```bash
export DATABRICKS_CONFIG_PROFILE=<your-profile-name> # e.g. cyber-mcp-server
...
databricks sync . "/Users/$DATABRICKS_USERNAME/dbx-cyber-mcp"
databricks apps deploy mcp-custom-server --source-code-path "/Workspace/Users/$DATABRICKS_USERNAME/dbx-cyber-mcp"
```
...
```yaml
command: ["uvicorn", "dbx_cyber_mcp.app:app"]
```
...
databricks bundle run cyber-mcp-server
```
